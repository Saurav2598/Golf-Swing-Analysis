...
def compute_impact_frame(ball_data, fs=120.0, threshold=1.0):
    ...
    # Detect first sustained velocity spike (2 consecutive frames above threshold)
    impact_frame = None
...
def plot_swing_analysis(
    ...
    if annotate:
        pel_i = int(np.argmax(np.abs(yaw_pelvis_f)))
        pel_v = yaw_pelvis_f[pel_i]
        
        tho_i = int(np.argmax(np.abs(yaw_thorax_f)))
        tho_v = yaw_thorax_f[tho_i]
        
        x_i = int(np.argmax(np.abs(xfactor_f)))
        x_v = xfactor_f[x_i]
    ...
    # Top: angles
    axes[0].plot(yaw_thorax_f, label="Thorax yaw")
    axes[0].plot(yaw_pelvis_f, label="Pelvis yaw")
    axes[0].plot(xfactor_f, label="X-factor")
    
    if p4_frame is not None:
        axes[0].axvline(int(p4_frame), linestyle="--", linewidth=1.5,
                        color="grey", label=f"P4: {int(p4_frame)}")
    if impact_frame is not None:
        axes[0].axvline(int(impact_frame), linestyle="--", linewidth=1.5,
                        color="black", alpha=0.7, label=f"Impact: {int(impact_frame)}")
    ...
    axes[0].set_title("Pelvis / Thorax Yaw and X-factor")
    ...
    if p4_frame is not None:
        axes[1].axvline(int(p4_frame), linestyle="--", linewidth=1.5,
                        color="grey", label=f"P4: {int(p4_frame)}")
    if impact_frame is not None:
        axes[1].axvline(int(impact_frame), linestyle="--", linewidth=1.5,
                        color="black", alpha=0.7, label=f"Impact: {int(impact_frame)}")
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=1.0, help="Ball speed threshold for impact detection in m/s (default: 1.0)")
    ...
    try:
        ...
        if impact_frame is not None:
            ...
            print(f"Impact Frame: Not detected")
